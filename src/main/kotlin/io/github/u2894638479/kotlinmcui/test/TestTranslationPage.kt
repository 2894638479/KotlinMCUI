...
fun TestTranslationPage() = ScrollableColumn {
    ...
    val stringProp = local { "test" }
    var string by stringProp
    val doubleProp = local { 0.5 }
    var double by doubleProp
    TextAutoFold {
        kotlinmcui.test_translate(string,(double*10).roundToInt(),double).emit()
    }
    Spacer(Modifier.height(10.scaled)) {}
    ...
    val customTranslationId = local { "kotlinmcui.testpage" }
    val keys by local { mutableListOf("") }

package io.github.u2894638479.kotlinmcui.dsl.decorator
...
import io.github.u2894638479.kotlinmcui.dsl.executeContext
...
= change { if(!enabled) it else object : DslComponent by it {
    ...
    fun click() {
        ...
        block(DslExecuteContext(),coroutineScope)
    }
...
fun DslChild.onHovered(
    ...
    object : DslComponent by it {
        ...
        override fun globalHoverChanged(newHover: DslId?) {
            ...
            action(executeContext,coroutineScope,instance.identity == newHover)
        }
    }
}
...
fun DslChild.onFocused(
    ...
    object: DslComponent by it {
        override fun globalFocusChanged(newFocus: DslId?) {
            ...
            action(executeContext,local.coroutineScope,instance.identity == newFocus)
        }
    }
}
...
fun DslChild.onFocusChanged(
    ...
    object: DslComponent by it {
        ...
        override fun globalFocusChanged(newFocus: DslId?) {
            it.globalHoverChanged(newFocus)
            action(executeContext,coroutineScope,newFocus)
        }
    }
}
...
fun DslChild.onHoverChanged(
    ...
    object: DslComponent by it {
        ...
        override fun globalHoverChanged(newHover: DslId?) {
            it.globalHoverChanged(newHover)
            action(executeContext,coroutineScope,newHover)
        }
    }
}
...
context(ctx: DslContext)
fun DslChild.onFilesDropped(action:context(DslExecuteContext) CoroutineScope.(Array<Path>) -> Unit) = change {
    object : DslComponent by it {
        val coroutineScope = local.coroutineScope
        context(mouse: Position)
        override fun dropFiles(files: Array<Path>): Boolean {
            if(mouse in instance.rect) {
                action(executeContext,coroutineScope,files)
                return true
            }
